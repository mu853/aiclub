...
#num_of_hidden_layers_list = [2, 3, 4, 5, 24, 30, 50, 100]
x = np.arange(24)

for i in range(8):
  model, optimizer = create_model(num_of_hidden_layers_list[i])
  for j in range(200): loss = train(model, optimizer, data, batch_size)

  for j in range(4):
    subplot_no = i * 4 + j + 1
    subp = plt.subplot(subplot_rows, subplot_cols, subplot_no)
    if j == 0:
      subp.set_title("k = %d, loss = %.2f" % (num_of_hidden_layers_list[i], loss))
    y = model.fwd(Variable(data)).data
    plt.plot(x, data[j * 20], color="r")
    plt.plot(x, y[j * 20], color="b")
